        ...
        switch (geom.type) {
            ...
            case 'MultiPolygon':
                ...
                for (var y = 0; y < iterations; y++) {
                    tempOutput = [[[]]];
                    poly = geom;
                    if (y > 0) poly = turf.multiPolygon(outCoords).geometry;
                    // console.log(poly)
                    processMultiPolygon(poly, tempOutput);
                    outCoords = tempOutput.slice(0);
                }
                // console.log(outCoords[0][0][0] === undefined ? [[]] : outCoords)
                // console.log(turf.multiPolygon(outCoords[0][0][0] === undefined ? [[]] : outCoords , properties))
                outPolys.push(turf.multiPolygon(outCoords[0][0][0] === undefined ? [[]] : outCoords, properties));
                break;
            default:
...
function processMultiPolygon(poly, tempOutput) {
    ...
    turf.coordEach(poly, function (currentCoord, coordIndex, featureIndex, multiFeatureIndex, geometryIndex) {
        ...
        try {
            ...
            // console.log(e)
            // console.log(multiFeatureIndex, geometryIndex, realCoordIndex + 1, poly.coordinates)
        }
    }, true);
...
turf.polygonSmooth = polygonSmooth